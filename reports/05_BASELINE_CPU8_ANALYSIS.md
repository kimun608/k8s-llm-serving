...
현재 Apple M4 로컬 환경에서 baseline의 Kubernetes container CPU limit만 `6`에서 `8`로 올리는 변경은 채택할 가치가 있다. 동일한 100개 prompt를 동시성 `1, 2, 5, 10, 20, 50, 100`에서 각각 실행한 결과, output throughput은 7개 구간 중 6개에서 `+11.7~24.8%` 개선됐다. 고정 출력 44,800 tokens의 phase 시간 합은 `4,598.07초 → 3,935.48초`로 14.4% 줄었고 합산 output throughput은 `9.74 → 11.38 token/s`, 즉 16.8% 증가했다.

다만 C=5에서는 throughput이 2.3% 낮고 E2E p95가 35.1% 높았다. 한 번씩 실행한 로컬 A/B이므로 CPU 8이 모든 부하에서 항상 빠르다고 일반화하지 않는다. 이번 워크로드와 장비에서 측정된 가장 단순하고 효과적인 한 필드 변경이라는 결론이며, production sizing 전에는 반복 측정이 필요하다.
...
| 5 | 11.28 → 11.01 | -2.3% | +35.1% | +2.9% | +0.5% | 5.55 → 6.66 | 5/0 → 5/0 |
...
### C=5: median 개선과 tail 악화가 동시에 발생한 변동 표본

C=5에서는 100개 중 75개의 개별 E2E가 빨라졌고 E2E p50도 16.3% 줄었다. 반면 마지막 쪽 batch의 TPOT가 길어져 E2E p95는 35.1% 늘고 phase throughput은 2.3% 낮았다. 두 설정 모두 waiting 0, peak KV 32.8%, preemption/OOM 0이어서 queue나 KV 부족으로 설명되지 않는다.

이 패턴은 CPU 8의 구조적 회귀라고 단정하기보다 Apple M4의 이종 core, Docker VM scheduling, host background load와 thermal state가 tail batch에 영향을 준 단일-run 변동으로 보는 것이 타당하다. 다만 이를 결과에서 제외하지 않았으며, 반복 A/B의 중앙값으로 확인하기 전까지 C=5 tail 개선은 입증되지 않은 것으로 판정한다.

## 자원과 안정성

| 지표 | CPU limit 6 | CPU limit 8 | 해석 |
|---|---:|---:|---|
| phase 평균 CPU의 평균 | 5.65 cores | 7.08 cores | 추가 quota가 실제 연산에 사용됨 |
...
CPU 8 Pod의 실험 후 누적 cgroup 값은 `nr_throttled/nr_periods=17,626/44,527`, 약 39.6%였다. 이는 warmup과 전체 실행이 섞인 누적값이라 phase별 A/B 변화율로 사용하지 않고, 9 OpenMP threads가 8-core quota 아래 있어 throttling 가능성이 여전히 남는다는 보조 증거로만 사용한다.
