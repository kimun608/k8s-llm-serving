...
| 5 | 11.28 → 11.01 | -2.3% | +35.1% | +2.9% |
| 10 | 12.06 → 14.94 | +23.9% | -19.3% | -21.6% |
| 20 | 13.50 → 16.22 | +20.2% | -16.2% | -22.6% |
| 50 | 13.46 → 15.03 | +11.7% | -9.6% | -11.7% |
| 100 | 13.54 → 15.15 | +11.8% | -10.4% | -8.8% |

고정 출력 44,800 tokens 기준 합산 output throughput은 `9.74 → 11.38 token/s`로 16.8% 증가했고 phase 시간 합은 14.4% 감소했다. C=5에서는 median 요청 75%가 빨라졌지만 tail batch가 느려져 p95와 phase 처리량이 악화됐다. 단일 로컬 run의 scheduling·thermal 변동 가능성이 있어 반복 검증 전까지 이 구간의 tail 개선은 입증되지 않은 것으로 본다.

장시간 실행 중 host가 중단된 초기 CPU 6 C=2와 CPU 8 C=10 표본은 결과에서 제외하되 `benchmark/results/*/excluded/`에 원시 데이터와 이유를 보존했다. 재측정된 정식 phase는 wall clock과 monotonic timer 차이가 모두 0.05초 이하다.
